...
async function getChapters(req, res) {
  ...
  try {
    const result = await pool.query(
      ...
      FROM a2_chapter c
      LEFT JOIN a2_listening_content l ON l.chapter_id = c.id
      LEFT JOIN a2_listening_progress p ON p.content_id = l.id AND p.user_id = $1
    ...
    console.error("Error fetching listening chapters:", err);
...
async function saveProgress(req, res) {
  ...
  try {
    await pool.query(
      `
      INSERT INTO a2_listening_progress (user_id, content_id, current_question_index, is_completed, score, last_practiced)
    ...
    console.error("Error saving listening progress:", err);
...
async function checkAnswers(req, res) {
  ...
  try {
    const result = await pool.query(
      `
      SELECT questions FROM a2_listening_content WHERE id = $1
    `,
      [contentId],
    );
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "true_false" || q.type === "truefalse") {
      ...
      } else if (q.type === "dialogue_dropdown") {
        // Check all dropdown selections
        if (userAnswer && typeof userAnswer === 'object') {
          isCorrect = q.dialogue.every((d, idx) => {
            if (d.text === null) {
        ...
        const correctOrder = q.question_data?.correct_order || q.correct_order || [];
    ...
    console.error("Error checking listening answers:", err);
