...
async function getChapters(req, res) {
  ...
  try {
    const result = await pool.query(
      ...
      FROM a1_chapter c
      LEFT JOIN a1_speaking_content s ON s.chapter_id = c.id
      LEFT JOIN a1_speaking_progress p ON p.chapter_id = c.id AND p.user_id = $1
    ...
    console.error("Error fetching A1 speaking chapters:", err);
...
async function getContent(req, res) {
  ...
  try {
    ...
    console.error("Error fetching A1 speaking content:", err);
...
async function saveProgress(req, res) {
  ...
  try {
    await pool.query(
      `
      INSERT INTO a1_speaking_progress (user_id, chapter_id, current_content_index, is_completed, last_practiced)
      VALUES ($1, $2, $3, $4, NOW())
    ...
    console.error("Error saving A1 speaking progress:", err);
...
async function saveAssessment(req, res) {
  ...
  try {
    await pool.query(
      `
      INSERT INTO a1_speaking_assessment (user_id, content_id, score, accuracy_score, fluency_score, pronunciation_score)
      VALUES ($1, $2, $3, $4, $5, $6)
    `,
      [
        userId,
    ...
    console.error("Error saving A1 speaking assessment:", err);
